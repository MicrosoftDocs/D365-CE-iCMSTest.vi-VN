...
ms.locfileid: "388187"
---
# <a name="use-powershell-cmdlets-for-xrm-tooling-to-connect-to-dynamics-365-for-customer-engagement"></a>Use PowerShell cmdlets for XRM tooling to connect to Dynamics 365 for Customer Engagement
...
- Đặt chính sách thực hiện để chạy các mã lệnh [!INCLUDE[pn_PowerShell_short](../../includes/pn-powershell-short.md)] đã ký. To do so, open a [!INCLUDE[pn_PowerShell_short](../../includes/pn-powershell-short.md)] window as an administrator and run the following command: `Set-ExecutionPolicy -ExecutionPolicy RemoteSigned`  
  
<a name="register"></a>   

## <a name="register-the-cmdlets"></a>Đăng ký các lệnh ghép ngắn  
...
## <a name="use-the-cmdlet-to-retrieve-organizations-from-includepndynamicscrmincludespn-dynamics-crmmd-apps"></a>Use the cmdlet to retrieve organizations from [!INCLUDE[pn_dynamics_crm](../../includes/pn-dynamics-crm.md)] apps 

 Use the `Get-CrmOrganizations` cmdlet to retrieve the organizations that you have access to.  
  
1. Cung cấp ủy nhiệm của bạn để kết nối với phiên bản [!INCLUDE[pn_crm_op_edition](../../includes/pn-crm-onprem.md)] hoặc [!INCLUDE[pn_CRM_Online](../../includes/pn-crm-online.md)] của bạn. Running the following command will prompt you to type your user name and password to connect to the [!INCLUDE[pn_crm_shortest](../../includes/pn-crm-shortest.md)] instance, and it will be stored in the `$Cred` variable.  
...
3. Cung cấp ủy nhiệm của bạn được xác nhận khi bạn chạy lệnh trong bước 2. On successful execution of the command, type the following command, and press ENTER to display the organizations that you have access to:  
...
## <a name="use-the-cmdlet-to-connect-to-includepndynamicscrmincludespn-dynamics-crmmd-apps"></a>Use the cmdlet to connect to [!INCLUDE[pn_dynamics_crm](../../includes/pn-dynamics-crm.md)] apps 
...
### <a name="connect-to-includepndynamicscrmincludespn-dynamics-crmmd-apps-by-using-the-common-login-control"></a>Connect to [!INCLUDE[pn_dynamics_crm](../../includes/pn-dynamics-crm.md)] apps by using the common login control  
...
### <a name="connect-to-includepndynamicscrmincludespn-dynamics-crmmd-apps-by-specifying-credentials-inline"></a>Connect to [!INCLUDE[pn_dynamics_crm](../../includes/pn-dynamics-crm.md)] apps by specifying credentials inline  
...
2. Cung cấp ủy nhiệm của bạn được xác nhận khi bạn chạy lệnh trong bước 1. On successful execution of the cmdlet, type the following command, and press ENTER to display the connection information and status:  
