...
ms.locfileid: "388410"
---
# <a name="entity-relationship-behavior"></a>Entity relationship behavior
...
-   [Bảo vệ tính toàn vẹn của dữ liệu](entity-relationship-behavior.md#BKMK_DataIntegrity)  
  
-   [Quy trình công việc tự động](entity-relationship-behavior.md#BKMK_BusinessProcesses)  
  
-   [Cascading behavior](entity-relationship-behavior.md#BKMK_CascadingBehavior)  
  
<a name="BKMK_DataIntegrity"></a>   
## <a name="preserve-data-integrity"></a>Bảo vệ tính toàn vẹn của dữ liệu  
...
## <a name="automate-business-processes"></a>Automate business processes  
...
- **Không phải làm gì.** The opportunities should continue to be owned by the individuals who work on them.  
...
## <a name="cascading-behavior"></a>Cascading behavior  
...
## <a name="parental-entity-relationships"></a>Mối quan hệ thực thể cấp độ cha  
 Mỗi cặp thực thể đủ điều kiện có một mối quan hệ 1:N có thể có nhiều mối quan hệ 1:N giữa chúng. Yet only one of those relationships can be considered a *parental* entity relationship. See [Customizable parental entity relationships](https://technet.microsoft.com/library/dn531061.aspx) for a list of all the customizable parental entity relationships.  
...
|------------|--------------|------------------|  
|**Xóa**|Cascade|Loại bỏ Liên kết<br />Hạn chế|  
|**Chia sẻ**|Cascade<br />UserOwned<br />Hoạt động|NoCascade|  
|**Không chia sẻ**|Cascade<br />UserOwned<br />Hoạt động|NoCascade|  
|**Gán**|Cascade<br />UserOwned<br />Hoạt động|NoCascade|  
|**Đổi mục mẹ**|Cascade<br />UserOwned<br />Hoạt động|NoCascade|  
...
## <a name="configure-entity-relationship-behavior"></a> Đặt cấu hình hành vi của mối quan hệ thực thể  
...
|Value|Application label|Mô tả|  
|-----------|-----------------------|-----------------|  
|Hoạt động|Phân tầng mục hiện hoạt|Perform the action on all active referencing entity records associated with the referenced entity record.|  
|Cascade|Phân tầng tất cả|Perform the action on all referencing entity records associated with the referenced entity record.|  
|NoCascade|Không phân tầng|Không phải làm gì.|  
|Loại bỏ Liên kết|Loại bỏ liên kết|Remove the value of the referencing attribute for all referencing entity records associated with the referenced entity record.|  
|Hạn chế|Hạn chế|Prevent the Referenced entity record from being deleted when referencing entities exist.|  
|UserOwned|Phân tầng mục do người dùng sở hữu|Perform the action on all referencing entity records owned by the same user as the referenced entity record.|  
...
|Hành động|Mô tả|Valid options|  
|------------|-----------------|-------------------|  
|Chỉ định|The referenced entity record owner is changed.|Hoạt động<br />Cascade<br />NoCascade<br />UserOwned|  
|Xóa|The referenced entity record is deleted. **Note:**  The options for this action are limited.|Cascade<br />Loại bỏ Liên kết<br />Hạn chế|  
|Hợp nhất|The record is merged with another record. **Note:**  For referenced entities that can be merged, Cascade is the only valid option. In other cases use NoCascade.|Cascade<br />NoCascade|  
|Đổi mục mẹ|See [About the Reparent action](entity-relationship-behavior.md#BKMK_ReparentAction) below.|Hoạt động<br />Cascade<br />NoCascade<br />UserOwned|  
|Chia sẻ|When the referenced entity record is shared with another user.|Hoạt động<br />Cascade<br />NoCascade<br />UserOwned|  
|Không chia sẻ|When sharing is removed for the referenced entity record.|Hoạt động<br />Cascade<br />NoCascade<br />UserOwned|  
  
<a name="BKMK_ReparentAction"></a>   
### <a name="about-the-reparent-action"></a>About the reparent action  
