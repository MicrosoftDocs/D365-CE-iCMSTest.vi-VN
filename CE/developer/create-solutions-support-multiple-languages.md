...
ms.locfileid: "386077"
---
# <a name="create-solutions-that-support-multiple-languages"></a>Create solutions that support multiple languages
...
|------------|-----------------------------|  
|[String (RESX) web resources](#BKMK_Localizable_Web_Resources)|Tài nguyên web|  
|[Embedded labels](#BKMK_EmbeddedLabels)|Application Navigation (SiteMap) <br />Ribbons|  
|[Export and import translations](#BKMK_ExportAndImportTranslations)|Thuộc tính<br />Biểu đồ<br />Bảng thông tin<br />Thực thể<br />Entity Relationships<br />Biểu mẫu<br />Messages<br />Option Sets<br />Dạng xem|  
|[Localization in base language strings](#BKMK_LocalizationInBaseLanguageStrings)|Contract Templates<br /> Connection Roles<br />Processes (Workflow)<br />Vai trò Bảo mật<br />Cấu hình Bảo mật Trường|  
|[Localization not required](#BKMK_LocalizationNotRequired)|SDK Message Processing Steps<br />Service Endpoints|  
|[Separate component for each language](#BKMK_SeparateComponentForEachLanguage)|Mẫu Bài viết<br />Các mẫu email<br />Mail Merge Templates<br />Báo cáo<br />Hộp thoại|  
...
## <a name="embedded-labels"></a>Embedded labels  
...
### <a name="sitemap"></a>SiteMap  
 ...
 The `<SubArea>` element allows for passing the user’s language preference by using the `userlcid` parameter so that content that is the target of the `SubArea.Url` attribute can be aware of the user’s language preference and adjust accordingly. 
...
## <a name="export-and-import-translations"></a>Export and import translations  
 Localizable labels for the solution components in the following table can be exported for localization.  
  
||||  
|-|-|-|  
|Thực thể|Thuộc tính|Mối quan hệ|  
|Global Option Sets|Thông báo của Thực thể|Biểu mẫu Thực thể|  
|Entity Views (SavedQuery)|Biểu đồ|Bảng thông tin|  
  
### <a name="translating-labels-and-display-strings"></a>Translating labels and display strings  
...
#### <a name="exporting-translations"></a>Exporting translations  
...
#### <a name="translating-text"></a>Translating text  
...
|Worksheet|Mô tả|  
|---------------|-----------------|  
|Thông tin|Displays information about the organization and solution that the labels and strings were exported from.|  
...
|Localized Labels|Displays all of the text for any metadata component labels.|  
 ...
 Bạn có thể gửi tập tin này đến một chuyên gia ngôn ngữ, đại lý cung cấp dịch vụ dịch thuật hoặc công ty địa phương hóa. They will need to provide localized strings for any of the empty cells.  
...
#### <a name="importing-translated-text"></a>Importing translated text  
  
> [!IMPORTANT]
>  You can only import translated text back into the same organization it was exported from.  
  
 After you have exported the customized entity or attribute text and had it translated, you can import the translated text strings in the web application by using the <xref:Microsoft.Crm.Sdk.Messages.ImportTranslationRequest> message. Tệp mà bạn nhập phải là tệp nén có chứa tệp CrmTranslations.xml và tệp the [Content_Types] ở dạng gốc. For more information, see [Import Translated Entity and Field Text](../customize/import-translated-entity-field-text.md).  
...
## <a name="localization-not-required"></a>Localization not required  
...
## <a name="separate-component-for-each-language"></a>Separate component for each language  
...
#### <a name="create-a-localized-dialog-box"></a>Create a localized dialog box  
...
## <a name="use-xml-web-resources-as-language-resources"></a>Use XML web resources as language resources  
...
### <a name="see-also"></a>Xem thêm  
 ...
 [Tạo, cài đặt và cập nhật giải pháp được quản lý](create-install-update-managed-solution.md)   
