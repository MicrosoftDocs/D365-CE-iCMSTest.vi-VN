...
ms.locfileid: "387352"
---
# <a name="use-webhooks-to-create-external-handlers-for-server-events"></a>Use webhooks to create external handlers for server events
...
## <a name="register-a-webhook"></a>Register a webhook
...
|Mục  |Mô tả  |
|---------|---------|
|**Tên**|A unique name describing the web hook.|
...
|Loại  |Mô tả  |
|---------|---------|
|**HttpHeader**|Includes one or more key values pairs in the header of the http request.<br />Ví dụ:  <br />`Key1: Value1`<br />`Key2: Value2`|
|**WebhookKey**|Includes a query string using `code` as the key and a value required by the endpoint. When registering the web hook using the Plug-in Registration tool, only enter the value.<br />Ví dụ:  <br />`?code=00000000-0000-0000-0000-000000000001`|
|**HttpQueryString**|Includes one or more key value pairs as query string parameters.<br />Ví dụ:  <br />`?Key1=Value1&Key2=Value2`|
...
### <a name="query-webhook-registrations"></a>Query webhook Registrations
...
## <a name="register-a-step-for-a-webhook"></a>Register a step for a webhook
...
### <a name="execution-mode-and-debugging-your-web-hook-registration"></a>Execution mode and debugging your web hook registration

Your choice in registering the webhook changes the experience you will have when debugging if things don’t work.

#### <a name="asynchronous-mode"></a>Asynchronous mode
...
#### <a name="synchronous-mode"></a>Synchronous mode
...
#### <a name="possible-causes-for-failure"></a>Possible causes for failure
...
### <a name="query-steps-registered-for-a-webhook"></a>Query steps registered for a webhook
...
### <a name="query-failed-asynchronous-jobs-for-a-given-step"></a>Query failed asynchronous jobs for a given step
...
## <a name="test-your-registration-with-a-request-logging-site"></a>Test your registration with a request logging site
...
## <a name="create-or-configure-a-service-to-consume-webhook-requests"></a>Create or Configure a service to consume webhook requests
...
### <a name="data-passed-to-the-service"></a>Data passed to the service

There are three types of data in the request: Query String, Header Data, and Request Body.

#### <a name="query-string"></a>Chuỗi Truy vấn
...
#### <a name="header-data"></a>Header Data
...
|---------|---------|
|**Thông báo**|Update|
|**Thực thể chính**|người liên hệ|
|**Secondary Entity**|không|
...
## <a name="invoke-a-webhook-from-a-plugin-or-workflow-activity"></a>Invoke a webhook from a plugin or workflow activity
