...
ms.locfileid: "386452"
---
# <a name="use-fetchxml-to-construct-a-query"></a>Use FetchXML to construct a query
...
## <a name="create-the-query-string"></a>Create the Query String  
...
## <a name="execute-the-query"></a>Execute the Query  
...
## <a name="query-results"></a>Query Results  
 When you execute a FetchXML query by using the <xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceProxy>.<xref:Microsoft.Xrm.Sdk.Client.OrganizationServiceProxy.RetrieveMultiple(Microsoft.Xrm.Sdk.Query.QueryBase)> method authenticates the user. method, the return value is an <xref:Microsoft.Xrm.Sdk.EntityCollection> that contains the results of the query. You can then iterate through the entity collection. The previous example uses the `foreach` loop to iterate through the result collection of the FetchXML query.  
