...
ms.locfileid: "386789"
---
# <a name="use-web-api-actions"></a>Use Web API actions
...
## <a name="unbound-actions"></a>Unbound actions
...
## <a name="bound-actions"></a>Bound actions
...
## <a name="use-a-custom-action"></a>Use a custom action
...
### <a name="custom-action-example-add-a-note-to-a-contact"></a>Custom action example: Add a note to a contact
...
|UI Label|Value|
|--------------|-----------|
|**Tên quy trình**|AddNoteToContact|
|**Tên Duy nhất**|new_AddNoteToContact|
|**Thực thể**|người liên hệ|
|**Thể loại**|Hành động|

 **Process Arguments**

|Tên|Loại|Bắt buộc|Hướng|
|----------|----------|--------------|---------------|
|NoteTitle|Chuỗi|Bắt buộc|Đầu vào|
|NoteText|Chuỗi|Bắt buộc|Đầu vào|
|NoteReference|EntityReference|Bắt buộc|Đầu ra|

 **Bước**

|Tên|Step Type|Mô tả|
|----------|---------------|-----------------|
|Create the note|Tạo Bản ghi|Title = {NoteTitle(Arguments)}<br /> Note Body = {NoteText(Arguments)}<br /> Regarding = {Contact{Contact}}|
|Return a reference to the note|Gán giá trị|NoteReference Value = {Note(Create the note (Note))}|
...
## <a name="specify-entity-parameter-type"></a>Specify entity parameter type
