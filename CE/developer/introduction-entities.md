...
### <a name="names-used-in-entity-metadata"></a>Names used in entity metadata  
...
|Tên|Mô tả|  
|----------|-----------------|  
|Tên Hiển thị|The name that is displayed to the user.|  
...
|Tên Sơ đồ|The schema name is specified upon creation. It must be unique. It is used to create the logical name. This name should be in Pascal case. The schema name is used to create the class for the entity when using early bound programming.|  
...
## <a name="actions-on-entity-records"></a>Actions on entity records  
...
|Hành động|Mô tả|Method and/or Message|  
|------------|-----------------|----------------------------|  
|[Tạo](introduction-entities.md#Create)|Creates a record of a specific entity type, including custom entities.|<xref:Microsoft.Xrm.Sdk.IOrganizationService>.<xref:Microsoft.Xrm.Sdk.IOrganizationService.Create*><br /><br /> <xref:Microsoft.Xrm.Sdk.Messages.CreateRequest>|  
...
|[Xóa](introduction-entities.md#Delete)|Deletes a record.|<xref:Microsoft.Xrm.Sdk.IOrganizationService>.<xref:Microsoft.Xrm.Sdk.IOrganizationService.Delete*><br /><br /> <xref:Microsoft.Xrm.Sdk.Messages.DeleteRequest>|  
|[Truy xuất](introduction-entities.md#Retrieve)|Retrieves a record.|<xref:Microsoft.Xrm.Sdk.IOrganizationService>.<xref:Microsoft.Xrm.Sdk.IOrganizationService.Retrieve*><br /><br /> <xref:Microsoft.Xrm.Sdk.Messages.RetrieveRequest>|  
...
|[Gán](introduction-entities.md#Assign)|Changes ownership of a record. Valid for user-owned or team-owned entities.|<xref:Microsoft.Crm.Sdk.Messages.AssignRequest>|  
|[Chia sẻ](introduction-entities.md#Share)|Grants, modifies or revokes access to a record to another user or team. Valid for user-owned or team-owned entities.|<xref:Microsoft.Crm.Sdk.Messages.GrantAccessRequest><br /><br /> <xref:Microsoft.Crm.Sdk.Messages.ModifyAccessRequest><br /><br /> <xref:Microsoft.Crm.Sdk.Messages.RevokeAccessRequest>|  
|[Liên kết](introduction-entities.md#Associate)|Creates links between a record and a collection of records where there is a relationship between the entities.|<xref:Microsoft.Xrm.Sdk.Messages.AssociateRequest>|  
|[Dừng liên kết](introduction-entities.md#Disassociate)|Removes links between a record and a collection of records where there is a relationship between the entities.|<xref:Microsoft.Xrm.Sdk.Messages.DisassociateRequest>|  
...
### <a name="retrieve-multiple"></a>Retrieve multiple  
 To perform this action, the caller must have Retrieve Privileges and access rights on the entity records retrieved.  
...
## <a name="entity-ownership"></a>Quyền sở hữu thực thể  
...
|Người dùng hoặc nhóm được sở hữu|Assigned to a user or to a team. These entities contain data that relates to customers, such as accounts or contacts. Security can be defined according to the business unit for the user or team. These entities have attributes named `owningteam` and `owninguser`.|  
|Không có|These entities are not owned by another entity.|  
...
## <a name="record-state-and-status"></a>Record state and status  
...
## <a name="entity-images"></a>Entity images  
...
|                    |                     |                     |
|--------------------|---------------------|---------------------|
|  Account **\\**\*  |      KbArticle      |      Chiến dịch       |
|      Sự cố      | Competitor **\\**\* |     Connection      |
|  Contact **\\**\*  |      Hợp đồng       | TransactionCurrency |
| Cấu hình Máy chủ Email |        Mục tiêu         |       Hóa đơn       |
|   Lead **\\**\*    |       Hộp thư       | OpportunityProduct  |
|     SalesOrder     |    Tổ chức     |  Product **\\**\*   |
| Publisher **\\**\* |        Hàng đợi        |  Resource **\\**\*  |
|  SalesLiterature   |      Vùng lãnh thổ      | SystemUser **\\**\* |
