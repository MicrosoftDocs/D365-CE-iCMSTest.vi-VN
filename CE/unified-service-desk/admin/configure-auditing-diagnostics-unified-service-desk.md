...
ms.locfileid: "382407"
---
# <a name="auditing-and-diagnostics-overview"></a>Auditing and diagnostics overview
...
### <a name="audit-and-diagnostics-configuration-overview"></a>Audit and diagnostics configuration overview  
...
#### <a name="create-an-audit--diagnostics-record-to-use-for-auditing"></a>Create an Audit & Diagnostics record to use for auditing  
...
#### <a name="standard-auditing-by-adding-an-audit-flag"></a>Standard auditing by adding an audit flag  
...
4. Trên trang tùy chọn, bấm vào **Mới**.  
...
5. On the **New Option** page, type an audit flag name in the **Name** field and an appropriate value in the **Value** field.  
   ...
   |         Cờ kiểm tra         |                                                                                        Mô tả                                                                                         |
   |----------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
   |     AuditCacheMaxSize      | When this flag value is set to **1**, the number of audit records cached before saving to the server is 1 record.<br /><br /> Tùy chọn này chỉ hoạt động nếu **AuditNoCache** là **sai**. |
   |      AuditFlagAction       |                                                  When this flag value is set to **1**, audit records are created when an action is fired.                                                  |
   |    AuditFlagAgentState     |                                            Khi giá trị cờ này được đặt thành **1**,tệp kiểm tra được tạo khi trạng thái tổng đài viên thay đổi.                                             |
   ...
   |       AuditFlagLogin       |                                                   Khi giá trị cờ này được đặt thành **1**, hồ sơ kiểm tra được tạo ra khi tổng đài viên đăng nhập.                                                   |
   |      AuditFlagSession      |                                  Khi giá trị cờ này được đặt thành **1**, hồ sơ kiểm tra được tạo ra khi một phiên làm việc được tạo ra hoặc có sự chuyển đổi phiên.                                   |
   |     AuditFlagWorkflow      |                                         Khi giá trị cờ này được đặt thành **1**, hồ sơ kiểm tra được tạo ra khi luồng công việc UII bắt đầu hoặc đóng.                                          |
...
#### <a name="create-an-audit--diagnostics-record-to-use-for-diagnostics"></a>Create an Audit & Diagnostics record to use for diagnostics  
...
5. Select from the available diagnostics options described here.  
   ...
   - **Diagnostics Logs Directory.** Specifies the full path to the folder where diagnostics files for exceptions are kept. If the path is invalid or inaccessible, [!INCLUDE[pn_unified_service_desk](../../includes/pn-unified-service-desk.md)] will use the default folder. Theo mặc định, thư mục là `%APPDATA%\Roaming\Microsoft\Microsoft Dynamics 365 for Customer Engagement Unified Service Desk\\<version\>\Diagnostics\\`.  
...
<a name="userschema"></a>   
## <a name="add-user-information-to-audit--diagnostics-settings-logging"></a>Add user information to Audit & Diagnostics Settings logging  
...
<a name="Deactivate"></a>   
## <a name="deactivate-or-activate-an-audit--diagnostics-settings-record"></a>Deactivate or activate an Audit & Diagnostics Settings record  
...
## <a name="see-also"></a>Xem thêm  
 ...
 [Administer and manage Unified Service Desk](../../unified-service-desk/admin/administer-manage-unified-service-desk.md) [Performance data collection using keyboard shortcut](../../unified-service-desk/admin/performance-data-collection-using-keyboard-shortcut.md)
